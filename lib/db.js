...
function openDb() {
  ...
  if (!migrated) {
    try { db.exec("ALTER TABLE articles ADD COLUMN sections TEXT NOT NULL DEFAULT '[]'"); } catch {}
    try { db.exec("ALTER TABLE articles ADD COLUMN faqs TEXT NOT NULL DEFAULT '[]'"); } catch {}
    try { db.exec("ALTER TABLE articles ADD COLUMN youtube_video_id TEXT NOT NULL DEFAULT ''"); } catch {}
    db.exec(`
...
function getTranslation(articleId, lang) {
  const db = openDb();
  return db.prepare('SELECT * FROM article_translations WHERE article_id = ? AND lang = ?').get(articleId, lang) || null;
}

function searchArticlesWithTranslations(lang, { page = 1, limit = 20 } = {}) {
  const db = openDb();
  const offset = (page - 1) * limit;
  const articles = db.prepare(`
    SELECT a.${ARTICLE_COLS}, t.headline AS translated_headline
    FROM articles a
    LEFT JOIN article_translations t ON t.article_id = a.id AND t.lang = ?
    ORDER BY a.created_at DESC LIMIT ? OFFSET ?
  `).all(lang, limit, offset);
  const total = db.prepare('SELECT COUNT(*) AS n FROM articles').get()?.n ?? 0;
  return { articles, total, page, pages: Math.max(1, Math.ceil(total / limit)) };
}

module.exports = { searchArticles, searchArticlesWithTranslations, getArticleById, getRelatedArticles, getNextArticle, getTranslation, getCategories, getDistinctSources, rateArticle };
