...
function openDb() {
  ...
  // Backfill slugs for any articles that don't have one yet
  const unslugged = _db.prepare("SELECT id, summary FROM articles WHERE slug = ''").all();
  if (unslugged.length) {
    const update = _db.prepare('UPDATE articles SET slug = ? WHERE id = ?');
    _db.transaction(() => {
      for (const row of unslugged) {
        const headline = (row.summary || '').split('\n').find(Boolean) || '';
        update.run(slugify(headline), row.id);
      }
    })();
  }
...
function getArticleById(id) {
  ...
  // Lazy slug generation for articles inserted before slug support
  if (row && !row.slug) {
    const headline = (row.summary || '').split('\n').find(Boolean) || '';
...
function getArticleBySlug(slug) {
  const db = openDb();
  const row = db.prepare(`SELECT ${ARTICLE_COLS} FROM articles WHERE slug = ?`).get(slug);
  if (row) return parseArticle(row);
  // Fallback: extract numeric ID from the end of the slug (format: title-123)
  const match = slug.match(/-(\d+)$/);
  if (match) {
    return parseArticle(db.prepare(`SELECT ${ARTICLE_COLS} FROM articles WHERE id = ?`).get(Number(match[1]))) || null;
  }
  return null;
}
