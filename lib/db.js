...
function openDb() {
  ...
  // Ensure schema exists on fresh DB (Railway build / first boot)
  _db.exec(`
...
function getTranslation(articleId, lang) {
  const db = openDb();
  return db.prepare('SELECT summary FROM translations WHERE article_id = ? AND lang = ?').get(articleId, lang)?.summary || null;
}

function saveTranslation(articleId, lang, summary) {
  const db = openDb();
  db.prepare('INSERT OR REPLACE INTO translations (article_id, lang, summary) VALUES (?, ?, ?)').run(articleId, lang, summary);
}

module.exports = { openDb, searchArticles, getArticleById, getArticleBySlug, getRelatedArticles, getNextArticle, getCategories, getDistinctSources, rateArticle, getTranslation, saveTranslation };
