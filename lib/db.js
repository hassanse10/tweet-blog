...
function searchArticles({ q, source, category, page = 1, limit = 20 } = {}) {
  ...
  if (needsSlug.length > 0) {
    const existingSet = new Set(
      db.prepare("SELECT slug FROM articles WHERE slug != ''").all().map((r) => r.slug)
    );
...
function getArticleBySlug(slug) {
  ...
  // 3. DB has old ID-suffixed slug (title-123), URL has the clean version (title)
  //    Find candidates with this prefix and filter for purely numeric suffix
  const candidates = db.prepare(
    `SELECT ${ARTICLE_COLS} FROM articles WHERE slug LIKE ?`
  ).all(`${slug}-%`);
  const escaped = slug.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  row = candidates.find((r) => new RegExp(`^${escaped}-\\d+$`).test(r.slug)) || null;
  if (row) {
    // Migrate slug to clean format if the clean slug is not already taken
    const conflict = db.prepare('SELECT id FROM articles WHERE slug = ?').get(slug);
    if (!conflict) {
      db.prepare('UPDATE articles SET slug = ? WHERE id = ?').run(slug, row.id);
      row.slug = slug;
    }
    return parseArticle(row);
  }
